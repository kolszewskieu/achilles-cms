...
if($_POST[zapisz]){
	...
	if(!$_POST[format_wklejka])$bug[format_wklejka]="<br/><span class='label label-important'>Brak formatu oklejki.</span>";
	...
	if($bug){
		...
		/*if($_POST[zapisz]=="add"){
			...
			$sql.="(id_format_wklejka,folia_x,folia_y,sztuk_arkusz,typ,cena,waluta) VALUES ";
			$sql.="('$_POST[format_wklejka]','$_POST[folia_x]','$_POST[folia_y]','$_POST[sztuk_arkusz]','$_POST[typ]','$_POST[cena]','$_POST[waluta]')";
			if(!mysql_query($sql)){
		...
		if($_POST[zapisz]=="edit"){
			...
			$sql.="folia_x='$_POST[folia_x]',folia_y='$_POST[folia_y]',
					...
					<div class="row-fluid">
						...
						<input type="hidden" name="site" value="<?=$_GET[site]?>">
							...
							<div class="row-fluid">
								<div class="span12">
									...
									<?select_typ_folie();?>
									<?select_typ();?>
									<span class='label label-important'>Dodawanie foli wklejki odbywa się przez dodanie formatu wklejki.</span>
						...
						<div class="span12">
							...
							<div class="row-fluid">
								<div class="span12">
									<?
										$sql="SELECT id_format,format_x_od,format_x_do,format_y_od,format_y_do,papier_x,papier_y,grubosc_od,grubosc_do FROM format_wklejka WHERE del='0'";
										$res=mysql_query($sql);									
										while($row=mysql_fetch_array($res)){
											...
											$lists_wklejka_format[$row[id_format]]=$text;											
										}
									...
									<input type="hidden" name="typ" value="<?=$_GET[typ];?>">									
										...
										if($_GET[add]){?>
											<tr>
											<td>
												Format wklejki: &nbsp;<select name="format_wklejka">
												<?
												foreach($lists_wklejka_format as $key => $val){
													echo "<option ";
													if($key==$_POST[id_format_wklejka])echo " selected ";
										...
										$sql="SELECT * FROM format_folia_wklejka WHERE typ='$_GET[typ]' AND typ_folie='$_GET[typ_folie]' AND del='0' ORDER BY id_format_wklejka";
										$res=mysql_query($sql);
										while($dane=mysql_fetch_array($res)){
											...
											?>
												...
												<td>
													Format wklejki: <br/>
													<?echo $lists_wklejka_format[$_POST[id_format_wklejka]];?>
													<?echo $bug[numer];?>												</td>
												<td>
													<input name="folia_x" value="<?=$_POST[folia_x]?>" data-source="[<?=$lists[folia_x]?>]" type="text" data-provide="typeahead" class="input-mini" autocomplete="off">&nbsp;x
													<input name="folia_y" value="<?=$_POST[folia_y]?>" data-source="[<?=$lists[folia_y]?>]" type="text" data-provide="typeahead" class="input-mini" autocomplete="off">
											...
											<?
												...
												echo "Format wklejka: ".$lists_wklejka_format[$dane[id_format_wklejka]];
