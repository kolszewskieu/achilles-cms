...
$result = mysql_query("SELECT Count(id) FROM `subkategorie_$cmslang`");
...
if($count_kateg > 0) {
	...
	if($_GET[prio_up]){
		...
		$sql="UPDATE `subkategorie_$cmslang` SET prio='$prio_tmp' WHERE prio=".$_GET[prio_up]."";
		mysql_query($sql);
		$sql="UPDATE `subkategorie_$cmslang` SET prio='".$_GET[prio_up]."' WHERE prio=$new_prio";
		mysql_query($sql);
		$sql="UPDATE `subkategorie_$cmslang` SET prio='$new_prio' WHERE prio=".$prio_tmp."";
		mysql_query($sql);
	}
	if($_GET[prio_down]){
		$prio_tmp=$count_kateg+1;
		$new_prio=$_GET[prio_down]+1;
		$sql="UPDATE `subkategorie_$cmslang` SET prio='$prio_tmp' WHERE prio=".$_GET[prio_down]."";
		mysql_query($sql);
		$sql="UPDATE `subkategorie_$cmslang` SET prio='".$_GET[prio_down]."' WHERE prio=$new_prio";
		mysql_query($sql);
		$sql="UPDATE `subkategorie_$cmslang` SET prio='$new_prio' WHERE prio=".$prio_tmp."";
		mysql_query($sql);	
	}
	if ($action == 'zapisz') {		
		...
		$sql = "UPDATE `subkategorie_$cmslang` SET `nazwa` = '".$_POST[nazwa]."', `opis` = '".$_POST[opis]."', `widocznosc` = '".$_POST[widocznosc]."' WHERE `id` = '".$kategoria."'";		
		//echo $sql;
	...
	if ($action == 'usun') {		
		$sql = "DELETE FROM `subkategorie_$cmslang` WHERE `id` = '".$kategoria."'";		
		$result = mysql_query($sql);
    ...
    $result = mysql_query("SELECT * FROM `subkategorie_$cmslang` ORDER BY ".$sort." LIMIT ".($per_page*($current_page-1)).", ".$per_page);
    while($row = mysql_fetch_assoc($result)) {
		...
		echo  '</td><td>'.$row['nazwa'].'</td><td style="width:80px;"';
				$dodaj = '';
