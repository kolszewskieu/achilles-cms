...
if($_POST[typ_kopia] && $_POST[zapisz]!="add"){
	copy_in_table("druk_oklejka",$_GET[typ],$_POST[typ_kopia],$_POST[id_copy]);
}
...
if($_GET[del_id]){
	$sql="UPDATE druk_oklejka SET del='1' WHERE id=$_GET[del_id]";
	if(!mysql_query($sql)){
						...
						<input type="hidden" name="site" value="<?=$_GET[site]?>">
							...
							<div class="row-fluid">
								<div class="span12">
									<legend>Druk oklejka&nbsp;&nbsp;&nbsp;
									<?select_typ();?>
									<a class="btn btn-mini btn-primary" role="button" href="?site=<?=$_GET[site]?>&typ=<?=$_GET[typ]?>&add=1">Dodaj druk oklejki</a>
									</legend>
								</div>
							</div>
							</fieldset>
								...
								<div class="span12">
									<?
										if($_GET[edit_id] || $_GET[add]){									
											...
											foreach($lists as $key => $val){
												$sql="SELECT DISTINCT $key as dana FROM druk_oklejka WHERE del='0'";
												$res=mysql_query($sql);									
									...
									<form name="edycja" method="POST" action="?site=<?=$_GET[site]?>&typ=<?=$_GET[typ]?>">									
										...
										<thead align="center">
											...
											<th nowrap="1"><a href="?site=<?=$_GET[site]?>&sort=sztuk_arkuszsztuk_arkusz&sortt=<?=($_GET[sortt]=="DESC"?"ASC":"DESC")?>">Szt arkusz</a>
											<?if($_GET[sort]=="sztuk_arkusz" && $_GET[sortt]=="ASC"){?><i class="icon-chevron-up"></i><?}?>
											<?if($_GET[sort]=="sztuk_arkusz" && $_GET[sortt]=="DESC"){?><i class="icon-chevron-down"></i><?}?>
											</th>
											<th></th>
											<th>Edycja</th>
										</thead>
										...
										<?}
										$sql="SELECT * FROM druk_oklejka WHERE typ='$_GET[typ]' AND del='0' ";
