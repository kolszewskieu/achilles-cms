...
if($_POST[zapisz]){
	...
	if($bug){
		...
		if($_POST[zapisz]=="add"){
			$sql="INSERT INTO format_wklejka ";
			...
			if(!mysql_query($sql)){
				$alert="Dodanie formatu wklejki nie powiodło się <br/>".mysql_error();
			}else{
				$sql_prio="UPDATE format_wklejka SET prio=prio+1 WHERE typ=".$_GET[typ]." AND prio>=".$_POST[prio]." AND prio<1000".$_POST[prio]."" ;
				mysql_query($sql_prio);
				$sql_prio="UPDATE format_wklejka SET prio=$_POST[prio] WHERE typ=".$_GET[typ]." AND prio=1000".$_POST[prio];
				mysql_query($sql_prio);

				$alert_ok="Format wklejki został dodany do bazy.";
				$_GET[typ]=$_POST[typ];
			}
			$sql="SELECT MAX(id_format) FROM format_wklejka";
			list($id_format_wklejka_last)=mysql_fetch_row(mysql_query($sql));
			...
			while($dane=mysql_fetch_array($res)){
				$sql_ins_folia="INSERT INTO format_folia_wklejka (id_format_wklejka,folia_x,folia_y,sztuk_arkusz,typ,typ_folie,cena,waluta) VALUE ('$id_format_wklejka_last','$folia_x_new','$folia_y_new','$sztuk_arkusz_new','$_POST[typ]','$dane[id]','$dane[cena]','$dane[waluta]') ";
				if(!mysql_query($sql_ins_folia)){
					$alert.="<br>Dodanie foli wklejki nie powiodło się <br/>".mysql_error();
				}else{
					$alert_ok.="<br>Folia wklejki został dodany do bazy.";
				}				
			}
		}
		if($_POST[zapisz]=="edit"){
			$sql="UPDATE format_wklejka SET ";
			...
			if(!mysql_query($sql)){
				$alert="Edycja formatu wklejki nie powiodła się <br/>".mysql_error()."<br/>".$sql;
			}else{
				$alert_ok="Format wklejki zaktualizowany.";
				$_GET[typ]=$_POST[typ];
				...
				$res=mysql_query($sql);
				while($dane=mysql_fetch_array($res)){
					$sql_ins_folia="UPDATE format_folia_wklejka SET folia_x='$folia_x_new',folia_y='$folia_y_new',sztuk_arkusz='$sztuk_arkusz_new' WHERE id_format_wklejka='$_POST[edit_id_format]' AND del='0'";
					if(!mysql_query($sql_ins_folia)){
						$alert.="<br>Aktualizacja folii wklejki nie powiodło się <br/>".mysql_error();
					}else{
						$alert_ok.="<br>Folia wklejki został zaktualizowana.";
					}				
				}
			}
		}
...
if($_GET[prio_up]){
	...
	$sql="UPDATE format_wklejka SET prio='$prio_tmp' WHERE typ=".$_GET[typ]." AND prio=".$_GET[prio_up]."";
	mysql_query($sql);
	$sql="UPDATE format_wklejka SET prio='".$_GET[prio_up]."' WHERE typ=".$_GET[typ]." AND prio=$new_prio";
	mysql_query($sql);
	$sql="UPDATE format_wklejka SET prio='$new_prio' WHERE typ=".$_GET[typ]." AND prio=".$prio_tmp."";
	mysql_query($sql);
}
if($_GET[prio_down]){
	$prio_tmp=$_GET[count_kateg]+1;
	$new_prio=$_GET[prio_down]+1;
	$sql="UPDATE format_wklejka SET prio='$prio_tmp' WHERE typ=".$_GET[typ]." AND prio=".$_GET[prio_down]."";
	mysql_query($sql);
	$sql="UPDATE format_wklejka SET prio='".$_GET[prio_down]."' WHERE typ=".$_GET[typ]." AND prio=$new_prio";
	mysql_query($sql);
	$sql="UPDATE format_wklejka SET prio='$new_prio' WHERE typ=".$_GET[typ]." AND prio=".$prio_tmp."";
	mysql_query($sql);
}
...
if($_GET[del_id_format]){
	//$sql="UPDATE format_wklejka SET del='1' WHERE id_format=$_GET[del_id_format]";
	$sql="DELETE FROM format_wklejka WHERE id_format=$_GET[del_id_format]";
	if(!mysql_query($sql)){
		$alert="Usunięcie wklejki nie powiodło się <br/>".mysql_error();
	}else{
		$alert_ok="Format wklejki został usunięty z bazy.";
		$sql = "UPDATE format_wklejka SET prio=prio-1 WHERE typ=".$_GET[typ]." AND prio > ".$_GET[del_prio];
		if(!mysql_query($sql)){
	...
	$sql="UPDATE format_folia_wklejka SET del='1' WHERE id_format_wklejka=$_GET[del_id_format]";
	if(!mysql_query($sql)){
		$alert.="<br/>Usunięcie folii wklejki nie powiodło się <br/>".mysql_error();
	}else{
		$alert_ok.="<br/>Folia wklejki została usunięta z bazy.";
	}
}
            ...
            <div class="span12">
					<div class="row-fluid">
						...
						<input type="hidden" name="site" value="<?=$_GET[site]?>">
							...
							<div class="row-fluid">
								<div class="span12">
									<legend>Format wklejek&nbsp;&nbsp;&nbsp;
									<?select_typ();?>
									<a class="btn btn-mini btn-primary" role="button" href="?site=<?=$_GET[site]?>&typ=<?=$_GET[typ]?>&add=1">Dodaj nową wklejkę</a>
									</legend>
								</div>
							</div>
							</fieldset>
									...
									<input type="hidden" name="typ" value="<?=$_GET[typ];?>">
										...
										if($_GET[edit_id_format] || $_GET[add]){
											...
											foreach($lists as $key => $val){
												$sql="SELECT DISTINCT $key as dana FROM format_wklejka WHERE del='0'";
												$res=mysql_query($sql);									
										...
										$sql="SELECT * FROM format_wklejka WHERE typ='$_GET[typ]' AND del='0' ORDER BY prio";
										$res=mysql_query($sql);
										...
										while($dane=mysql_fetch_array($res)){
											...
											<?
												...
												if($dane[prio]>1){
													echo '<a href="?site=wyceny_formaty_wklejka&typ='.$_GET[typ].'&prio_up='.$dane[prio].'&count_kateg='.$count.'"><i class="icon-hand-up"></i></a>';
												}
												echo'&nbsp;';	
												if($dane[prio]<$count){
													echo '<a href="?site=wyceny_formaty_wklejka&typ='.$_GET[typ].'&prio_down='.$dane[prio].'&count_kateg='.$count.'"><i class="icon-hand-down"></i></a>';
												}
												echo "</td><td>";
